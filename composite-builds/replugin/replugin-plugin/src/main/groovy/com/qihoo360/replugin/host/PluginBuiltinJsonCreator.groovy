...
public class PluginBuiltinJsonCreator implements IFileCreator {
    ...
    String getFileContent() {
        ...
        if (!pluginDirFile.exists()) {
            println "${Constants.TAG} The ${pluginDirFile.absolutePath} does not exist "
            println "${Constants.TAG} pluginsInfo=null"
            return null
        }
        ...
        new File(fileDir.getAbsolutePath() + File.separator + config.pluginDir)
            ...
            try {
                ...
                if (config.enablePluginFileIllegalStopBuild) {
                    System.err.println "${Constants.TAG} the plugin(${it.absoluteFile.absolutePath}) is illegal !!!"
        ...
        if (pluginInfos.isEmpty()) {
            println "${Constants.TAG} pluginsSize=0"
            println "${Constants.TAG} pluginsInfo=null"
            return null
        }
        ...
        //格式化打印插件们的json信息
        println "${Constants.TAG} pluginsSize=${pluginInfos.size()}"
        println "${Constants.TAG} pluginsInfo=${jsonOutput.prettyPrint(pluginInfosJson)}"
