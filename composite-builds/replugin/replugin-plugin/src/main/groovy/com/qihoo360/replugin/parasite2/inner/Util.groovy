...
package com.qihoo360.replugin.parasite2.inner
...
public class Util {
    ...
    static getClassPaths(Project project, Collection<TransformInput> inputs, Set<String> includeJars, Map<String, String> map) {
        ...
        // android.jar
        classpathList.add(com.qihoo360.replugin.util.Util.findSdkLocation(project).getAbsolutePath())
        ...
        inputs.each { TransformInput input ->
            ...
            input.jarInputs.each { JarInput jarInput ->
                ...
                if (!jarPath.contains(projectDir)) {
                    ...
                    if (unzip(project,jarPath, jarZipDir)) {
     ...
     */
    def static zipDir(def project,String dirPath, String zipFilePath) {
        File dir = new File(dirPath)
        if (dir.exists()) {
            project.ant.zip(destfile: zipFilePath, basedir: dirPath)
        } else {
     ...
     */
    def private static boolean unzip(def project ,String zipFilePath, String dirPath) {
        // 若这个Zip包是空内容的（如引入了Bugly就会出现），则直接忽略
        if (isZipEmpty(zipFilePath)) {
            println ">>> Zip file is empty! Ignore";
            return false;
        }
        project.ant.unzip(src: zipFilePath, dest: dirPath, overwrite: 'true')
        return true;
    }
