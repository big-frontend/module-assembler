...
local_propeties_path = opath.join(root_path, 'local.properties')
source_modules = []
exclude_modules = []
binary_modules = []
activeBuildVariant = ''
...
def main(argv):
    print(argv)
    global activeBuildVariant
    for a in argv:
        k, v = a.split("=")
        if k == 'activeBuildVariant':
            activeBuildVariant = v
        elif k == 'excludeModules':
            exclude_modules = v
            # print(exclude_modules)

    p = Properties()
    p.load(open(local_propeties_path))
    if activeBuildVariant:
        p['activeBuildVariant'] = activeBuildVariant
    for s in exclude_modules.split(','):
        if s not in p['excludeModules']:
            p['excludeModules'] += f'{s},'
    n = ''
    for s in p['sourceModules'].split(','):
        if s not in exclude_modules:
           n += f'{s},'
    p['sourceModules'] = n
    p.store(open(local_propeties_path,'w'))
